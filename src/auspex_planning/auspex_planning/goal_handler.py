...
from .planner.utils.goal_utils.nlp_llm import NLP_LLM
    ...
    def process_mission(self, team_id, mission):
        ...
        try:
            ...
            if mission['desired_ground_dist']:
                height = float(mission['desired_ground_dist'])
            else:
                height = (float(mission['max_height']) + float(mission['min_height'])) / 2 if mission['max_height'] and mission['min_height'] else 50.0

            # Compute the centroid of the given points
            lat_sum = sum(float(p['latitude']) for p in mission['search_area']['points'])
            lon_sum = sum(float(p['longitude']) for p in mission['search_area']['points'])
            num_points = len(mission['search_area']['points'])
            centroid = [lat_sum / float(num_points),lon_sum / float(num_points),height]
            area_str = json.dumps({
