...
class EnvDiscription:
    ...
    instruction_prompt = """\n**Instruction**:
                You are controlling a quadrocopter. The quadracopter has a rgb camera together with an object detection algorithm installed. With the following skills: {action_params_description} \n These are a set of skills. Some of them are useful to control a drone.\n
                This means, you are given a higher goal and you will then reason an adequat skill for the quadrocopter to execute.
                Include the Take off skill once to start the drone.
                **Every area can only be searched once.** 
                **DO NEVER INCLUDE NOT GOAL-RELEVANT SKILLS. If a skill is unhelpful to reach a goal just ignore it.** 
                **IMPORTANT: Avoid choosing skills which are not suitable for this scenario.**
                **Choose only skills which help the progress of the goal**
                Always evaluate if the selected skill is applicable in a quadcopter scenario. 
                After either reaching the goal, or the drone has visited all related areas use the "Return to Home and Land" skill to fly to the home position, land and temrinate the program.
                Please select the appropriate skills to achieve the goal, ensuring that the drone lands once the goal has been reached and no further actions are necessary.
                ...
                **Goal:**
                ...
                """
    look_up_action_list_without_params = ["Return to Home and Land.", "Take off", "Search an area", "Hover", "Confirm an object", "Ascend", "Descend", "Perform mapping", "Take one single image"]
    ...
    detection_color = ['blue', 'red', 'yellow', 'orange', 'green', 'purple', 'white', 'black']
    high_level_skill = ['Search', 'Map', 'Transport', 'Surveill', 'Track']
    ...
    def NL2action_msg(self, action, num_actions):
        ...
        if "Land" in action:
            return ['fly_home_and_land']
        elif "Search" in action: 
            s1 = ['searchArea', 'ahp', 43, self.getObjectFromString(action), 'openareas1']
            s2 = ['searchArea', 'ahp', 43, self.getObjectFromString(action), 'openareas2']
            s = ['searchArea', 'ahp', 43, self.getObjectFromString(action), self.getWaypointFromString(action)]
            if num_actions == 2:
                return s
            else:
                return s
