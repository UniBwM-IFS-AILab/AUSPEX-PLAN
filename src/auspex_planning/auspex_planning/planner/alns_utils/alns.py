# FILE from https://github.com/N-Wouda/ALNS
...
class ALNS:
    ...
    def add_destroy_operator(
        ...
        """
        logger.info(f"Adding destroy operator {op.__name__}.")
    ...
    def iterate(
        ...
        **kwargs
        ...
        while not stop(self._rnd_state, best, curr):
            ...
            destroyed = d_operator(curr, self._rnd_state, **kwargs) #call to destroy operator
            cand = r_operator(destroyed, self._rnd_state, **kwargs) #call to repair operator

            end_time = time.time() ######
            duration = end_time - start_time  #####
            logger.debug(f"Time for the couple is {duration}")  #####

            best, curr, outcome = self._eval_cand(
                accept, best, curr, cand, **kwargs
            )

            op_select.update(cand, d_idx, r_idx, outcome,duration) #####just added the parameter
